...
class Records:
	...
	def unpack(self):
		...
		for i in range(self.items_count):
			...
			row_tuple = struct.unpack("<HQQHIII", b_row)
			
			
			
			
			# b_ftype = self.__buffer.read(2)
			# ftype = struct.unpack("<H", b_ftype)[0]
			...
			# self.__buffer.seek(8 + 8 + 2 + 4 + 4)
			# self.__buffer.read(8 + 8 + 2 + 4 + 4)					# TODO
			#
			# b_name_pos = self.__buffer.read(4)
			# name_pos = struct.unpack("<I", b_name_pos)[0]
			...
			record = Record()
			# record.ftype = ftype
			# record.name_pos = name_pos
			record.ftype = row_tuple[0]
			record.uid = row_tuple[4]
			record.parent_id = row_tuple[5]
			record.name_pos = row_tuple[6]
...
class Volume(object):
	...
	def load(self):
		...
		#--- records
		self.records.items_count = self.header.records_len
		self.records.set_bdata(fd.read(self.header.section_table_len))
		self.records.unpack()
		
		
		#--- texts
		section_texts = fd.read(self.header.section_text_len)
		print("section_texts need read:", self.header.section_text_len)
