...
class TreeFrame(tkinter.Frame):
	...
	def __make_tree(self):
		...
		volumes = self.storage.get_volumes()
	...
	def __find_childrens(self, volume_id, parent_id):
			result = [
				file for file in self.files if (file[FRow.PARENT] == parent_id) and (file[FRow.VOLUME] == volume_id)
			]
			return result

	def __wnode(self, volume_id, parent_id):
			"""
